...
def _instance():
    ...
    Functions ``resources``, ``load_font``, ``charmap``, ``font`` and
...
def icon(*names, **kwargs):
    """
    Return a QIcon object corresponding to the provided resources name(s).
    ...
    Such resources stacks are generally used to combine multiple glyphs to make
    more complex icons.
    ...
    the list of option dictionaries to be used for each glyph::
        ...
        camera_ban = qta.resources('fa5s.camera', 'fa5s.ban', options=[{
                'scale_factor': 0.5,
    ...
    Qt's ``QIcon`` object has four modes
        ...
        - ``Normal``: The user is not interacting with the resources, but the
          functionality represented by the resources is available.
        - ``Disabled``: The functionality corresponding to the resources is not
          available.
        - ``Active``: The functionality corresponding to the resources is available.
          The user is interacting with the resources, for example, moving the mouse
          over it or clicking it.
        - ``Selected``: The item represented by the resources is selected.
 
    The glyph for the Normal mode is the one specified with the main positional
    argument.

     - ``color``: resources color in the ``Normal`` mode.

    The following four options will apply to the resources regardless of the mode.

     - ``offset``: tuple (x, y) corresponding to the horizontal and vertical
       offsets for the glyph, specified as a proportion of the resources size.
     - ``animation``: animation object for the resources.
     - ``scale_factor``: multiplicative scale factor to be used for the glyph. 

    The following options apply to the different modes of the resources

     - ``active``: name of the glyph to be used when the resources is ``Active``.
     - ``color_active``: the corresponding resources color.

     - ``disabled``: name of the glyph to be used when the resources is ``Disabled``.
     - ``color_disabled``: the corresponding resources color.

     - ``selected``: name of the glyph to be used when the resources is ``Selected``.
     - ``color_selected``: the corresponding resources color.
...
def charmap(prefixed_name):
    ...
    -------
    return_value: dict
        The dictionary mapping the resources names to the corresponding unicode character.
...
class IconWidget(QtWidgets.QLabel):
    """
    IconWidget gives the ability to display an resources as a widget

    if supports the same arguments as resources()
    for example
