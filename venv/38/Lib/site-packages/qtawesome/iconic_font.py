...
def set_global_defaults(**kwargs):
    """Set global defaults for the options passed to the resources painter."""
...
class CharIconPainter:
    ...
    def _paint_icon(self, iconic, painter, rect, mode, state, options):
        ...
        # A 16 pixel-high resources yields a font size of 14, which is pixel perfect
        # for font-awesome. 16 * 0.875 = 14
        # The reason why the glyph size is smaller than the resources size is to
...
class IconicFont(QObject):
    ...
    def icon(self, *names, **kwargs):
        """Return a QIcon object corresponding to the provided resources name."""
            ...
            if '.' in name:
                ...
                if prefix in self.charmap:
                    if n in self.charmap[prefix]:
                        ...
                        error = 'Invalid resources name "{0}" in font "{1}"'.format(
                            n, prefix)
                        raise Exception(error)
                else:
                ...
                raise Exception('Invalid resources name')
    ...
    def set_custom_icon(self, name, painter):
        """Associate a user-provided CharIconPainter to an resources name.

        The custom resources can later be addressed by calling
        resources('custom.NAME') where NAME is the provided name for that resources.

        Parameters
        ----------
        name: str
            name of the custom resources
        painter: CharIconPainter
            The resources painter, implementing
            ``paint(self, iconic, painter, rect, mode, state, options)``
        """
        self.painters[name] = painter

    def _custom_icon(self, name, **kwargs):
        """Return the custom resources corresponding to the given name."""
    ...
    def _icon_by_painter(self, painter, options):
        """Return the resources corresponding to the given painter."""
