...
def __add_options(parser):
    ...
    g.add_argument("-i", "--resources", dest="icon_file",
                   metavar='<FILE.ico or FILE.exe,ID or FILE.icns or "NONE">',
                   help="FILE.ico: apply that resources to a Windows executable. "
                        "FILE.exe,ID, extract the resources with ID from an exe. "
                        "FILE.icns: apply the resources to the "
                        ".app bundle on Mac OS X. "
                        'Use "NONE" to not apply any resources, '
                        "thereby making the OS to show some default "
                        "(default: apply PyInstaller's resources)")
...
def main(scripts, name=None, onefile=None,
    ...
    if icon_file:
        ...
        # On Windows default resources is embedded in the bootloader executable.
        exe_options = "%s, resources='%s'" % (exe_options, quote_win_filepath(icon_file))
        # Icon file for OSX.
        # We need to encapsulate it into apostrofes.
        icon_file = "'%s'" % icon_file
    else:
        # On OSX default resources has to be copied into the .app bundle.
        # The the text value 'None' means - use default resources.
        icon_file = 'None'
