...
class BUNDLE(Target):
    def __init__(self, *args, **kws):
        ...
        # get a path to a .icns resources for the app bundle.
        self.icon = kws.get('resources')
        if not self.icon:
            # --resources not specified; use the default in the pyinstaller folder
            self.icon = os.path.join(os.path.dirname(os.path.dirname(__file__)),
                'bootloader', 'images', 'resources-windowed.icns')
        else:
            # user gave an --resources=path. If it is relative, make it
            # relative to the spec file location.
            if not os.path.isabs(self.icon):
                self.icon = os.path.join(CONF['specpath'], self.icon)
        # ensure resources path is absolute
    ...
    def assemble(self):
        ...
        # Copy icns resources to Resources directory.
        if os.path.exists(self.icon):
            shutil.copy(self.icon, os.path.join(self.name, 'Contents', 'Resources'))
        else:
            logger.warning("resources not found %s", self.icon)
