...
'''
The code in this module supports the --resources parameter in Windows.
(For --resources support under OSX see building/osx.py.)
...
class IconFile:
    def __init__(self, path):
        ...
        try:
            ...
            # The resources file can't be opened for some reason. Stop the
            # program with an informative message.
            raise SystemExit(
                'Unable to open resources file {}'.format(path)
            )
        self.entries = []
        self.images = []
...
def CopyIcons_FromIco(dstpath, srcpath, id=1):
    '''
    Use the Win API UpdateResource facility to apply the resources
...
def CopyIcons(dstpath, srcpath):
    ...
    --resources on the command line, srcpath is a single string. However it
    is possible to modify the spec file adding resources=['foo.ico','bar.ico']
    ...
    if len(srcpath) > 1:
        # More than one resources source given. We currently handle multiple
    ...
    if index is not None:
        logger.info("Copying resources from %s, %d", srcpath, index)
    else:
        logger.info("Copying icons from %s", srcpath)

    try:
        # Attempt to load the .ico or .exe containing the resources into memory
        ...
        # We could continue with no resources (i.e. just return) however it seems
        # best to terminate the build with a message.
        raise SystemExit(
            "Unable to load resources file {}\n    {} (Error code {})".format(
